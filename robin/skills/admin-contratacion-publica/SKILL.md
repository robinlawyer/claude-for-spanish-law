---
name: admin-contratacion-publica
description: >
  Asistencia en contratación pública (Ley 9/2017 LCSP): recursos especiales
  ante TACRC, redacción de ofertas, impugnación de adjudicaciones, prórrogas
  y modificaciones.
argument-hint: "[procedimiento + posición (licitador / adjudicatario / órgano) + materia]"
---

# /robin:admin-contratacion-publica

Pipeline:

1. **Identificar fase**: pliegos, oferta, adjudicación, ejecución,
   prórroga, modificación, resolución.

2. **Recurso especial en materia de contratación** (art. 44 LCSP):
   contratos sujetos a regulación armonizada (SARA) + contratos
   superiores a umbrales. Plazo 15 días hábiles.

3. **`mcp__robin__buscar_tacrc`** con la materia. Doctrina TACRC
   abundantísima.

4. **`mcp__robin__preparar_recurso_administrativo`** con
   tipo=tacrc.

5. **Si contrato no SARA**: recursos administrativos generales (Ley
   39/2015) o contencioso directo según norma.

6. **Modificación / prórroga / resolución**: requisitos de la LCSP.

7. **Costas en TACRC**: regla general no, pero TACRC puede imponer
   por mala fe.
...
- Suspensión automática del procedimiento durante el recurso TACRC
  (art. 53 LCSP) salvo levantamiento.
- Subcontratación: límites art. 215 LCSP.
- Modificaciones imprevistas: condicionadas a no alterar las
  condiciones esenciales (art. 205 LCSP).
