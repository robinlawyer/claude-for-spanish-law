---
name: civil-analizar-escrito
description: >
  Analiza un escrito procesal civil de la contraparte (demanda, contestación,
  recurso). Identifica argumentos, debilidades, citas a verificar, mejor
  estrategia de respuesta.
argument-hint: "[pega el escrito íntegro de la contraparte]"
---

# /robin:civil-analizar-escrito

Pipeline:

1. **`mcp__robin__analizar_escrito_contraparte`** con el texto
   completo. Devuelve clasificación de argumentos, citas extraídas,
   debilidades estructurales.

2. **Verifica TODAS las citas de la contraparte** con
   `mcp__robin__verificar_cita`. Lista citas:
   - Inventadas (no existe ECLI/BOE-A).
   - Derogadas (norma no vigente).
   - Mal aplicadas (sentencia que dice lo contrario de lo que se
     le atribuye).

3. **Busca jurisprudencia contraria** a los argumentos del actor.
   `/robin:jurisprudencia` con `--norma <referencia>` sobre los
   preceptos invocados.

4. **`mcp__robin__simular_oposicion`** para anticipar su réplica a
   una hipotética contestación.

5. **Estructura el output** como informe estratégico.

## Formato

```
📋 Análisis del escrito de la contraparte
...
Citas dudosas / inventadas
[tabla con cita / verificación / consecuencia explotable]

Líneas de ataque recomendadas (priorizadas)
1. [línea fuerte] — [norma + ECLI de apoyo]
2. […]

Jurisprudencia contraria (verificada)
[bloque por nivel jerárquico]

Anticipación: cómo replicará la contraparte
[escenarios y contramedidas]
```
