---
name: civil-monitorio
description: >
  Petición de procedimiento monitorio (arts. 812 ss LEC) para deudas dinerarias
  vencidas, exigibles y documentadas. Sin tope de cuantía desde 2011.
argument-hint: "[acreedor, deudor, importe, documentación]"
---

# /robin:civil-monitorio

Pipeline:

1. **Verifica idoneidad**:
   - Deuda dineraria, vencida, exigible.
   - Documentada en alguno de los modos del art. 812 LEC:
     a) Documento firmado por el deudor.
     b) Facturas, albaranes, certificaciones, telegramas, fax o
        similar habitual en relaciones comerciales.
     c) Documentos comerciales aunque no firmados, junto con prueba
        de la entrega.

2. **Jurisdicción y competencia**. Tribunal de Instancia (Sección
   Civil) del domicilio del deudor (art. 813 LEC; competencia
   integrada tras LO 1/2025).

3. **`mcp__robin__preparar_demanda`** con tipo=monitorio.

4. **Tasa**. `mcp__robin__tasa_judicial` (si persona jurídica).
   Personas físicas: exentas.

5. **Plazos de la otra parte**:
   - 20 días para pagar / oponerse (art. 815 LEC).
   - Si paga: cierre por satisfacción.
   - Si se opone: transforma a verbal (≤ 6.000 €) u ordinario (>).
   - Si silencio: ejecución directa (art. 816 LEC).

6. **Intereses**. Si la deuda lleva intereses moratorios (típico
   morosidad mercantil), incluirlos: `/robin:calculadora intereses`.

## Formato

Petición inicial muy breve (art. 814 LEC):

```
AL TRIBUNAL DE INSTANCIA DE [domicilio deudor], SECCIÓN CIVIL
...
- Si hay cláusula de sometimiento expreso a otro Tribunal de
  Instancia, no es válida en monitorio frente a consumidor
  (jurisprudencia TJUE sobre cláusulas abusivas).
- Si el deudor está en concurso → no procede monitorio, créditos
  van al TRLC.
