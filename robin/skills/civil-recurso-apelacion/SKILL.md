---
name: civil-recurso-apelacion
description: >
  Recurso de apelación contra sentencia de primera instancia. Errores in
  iudicando e in procedendo, motivos numerados, jurisprudencia de la AP/TS
  competente.
argument-hint: "[sentencia recurrida + motivos de discrepancia]"
---

# /robin:civil-recurso-apelacion

Pipeline:

1. **Plazo**. `/robin:plazos` — 20 días hábiles desde notificación
   (art. 458 LEC). Avisa si quedan pocos días.

2. **Depósito y tasa**. `mcp__robin__deposito_recurso` (50 € en
   apelación civil); `mcp__robin__tasa_judicial`.

3. **Foral-check** si procede.

4. **Identificar motivos** (in iudicando y/o in procedendo):
   - In iudicando: error en valoración de la prueba; error en
     aplicación del derecho material; incongruencia.
   - In procedendo: infracción de normas procesales; quebrantamiento
     del derecho a tutela judicial.

5. **Llama a `mcp__robin__preparar_recurso`** con tipo=apelacion. El
   orquestador estructura los motivos clasificados.

6. **Refuerza cada motivo** con `/robin:jurisprudencia`:
   - Filtra por AP territorial competente (la del recurso) + TS sobre
     el mismo precepto.
   - Jerarquía: TS unificador > AP local > otras AAPP.
   - Si hay STC sobre derechos fundamentales conexos, cítala
     primero.

7. **Si hay norma foral**: prioridad sobre CC estatal.
   `mcp__robin__buscar_por_ccaa`.

8. **Verifica TODAS las citas**.

9. **Persiste**.

## Formato

Estructura LEC arts. 458-465:

```
A LA ILMA. AUDIENCIA PROVINCIAL DE [territorio] QUE POR TURNO DE
REPARTO CORRESPONDA

Don/Doña [letrado], ICAM [nº], en nombre y representación de
[apelante] (parte [demandante/demandada] en la instancia)…

PREPARO RECURSO DE APELACIÓN contra la Sentencia nº [N] dictada en
el procedimiento [tipo, nº/año] por la Sección Civil del Tribunal de
Instancia de [partido judicial], Nº [N] (o, en su caso, por el
Juzgado de Primera Instancia nº [N] de [partido judicial] si la
resolución es anterior a la entrada en vigor de la LO 1/2025), el
[fecha], notificada el [fecha], en base a los siguientes

ANTECEDENTES PROCESALES

[Breve resumen del procedimiento y fallo]

MOTIVOS

Primer motivo. — Error en la valoración de la prueba (in iudicando).
[Norma + jurisprudencia con ECLI]

Segundo motivo. — Indebida aplicación de [artículo].
[Norma + jurisprudencia]

Tercer motivo. — Infracción procesal por […].
[Norma + jurisprudencia]

SUPLICO A LA AUDIENCIA

Que estimando el recurso, revoque la sentencia recurrida y, en su
lugar, […].

OTROSÍ DIGO: depósito de 50 € constituido conforme a DA 15ª LOPJ.
...
- Apelación contra sentencia dictada en juicio verbal de cuantía
  ≤ 3.000 € → no cabe apelación (art. 455.1 LEC).
- Si el recurso pretende revertir un hecho probado: justificar error
  patente o ausencia de motivación.
- Recursos contra autos: cauce de apelación distinto (apelación
  contra autos definitivos, queja contra los no definitivos).
