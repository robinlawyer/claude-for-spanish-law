---
name: civil-simular-oposicion
description: >
  Simula la oposición a un escrito propio antes de presentarlo. Detecta
  vulnerabilidades, argumentos de la contraparte previsibles, y refuerza el
  escrito.
argument-hint: "[escrito propio borrador]"
---

# /robin:civil-simular-oposicion

Pipeline:

1. **`mcp__robin__simular_oposicion`** con el escrito borrador.
   Devuelve los puntos que un letrado contrario competente atacaría.

2. **Para cada punto débil identificado**:
   - Busca jurisprudencia que sostenga o desmonte el ataque.
   - Propón refuerzo argumental o nueva cita.
   - Si el punto es procesalmente débil, sugiere mover el argumento
     o eliminarlo.

3. **Devuelve el escrito mejorado** marcado con cambios:
   - `+++ nuevo párrafo` para añadidos.
   - `--- párrafo a revisar` para puntos débiles.
   - `↻ párrafo sustituido por:` para reescrituras.

4. **Verifica TODAS las citas** del nuevo material.
...
3 puntos vulnerables detectados:
1. [Punto 1] — Riesgo ALTO. Razón: […]. Refuerzo propuesto: […]
2. [Punto 2] — Riesgo MEDIO.
3. [Punto 3] — Riesgo BAJO (mencionado por completitud).

Escrito mejorado con marcas de cambio:
[texto del escrito con +++ / --- / ↻]

Citas nuevas verificadas:
[tabla]
```
