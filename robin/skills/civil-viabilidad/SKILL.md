---
name: civil-viabilidad
description: >
  Análisis estratégico inicial de un caso civil. Cronología, mapa documental,
  viabilidad por escenario, plan estratégico y plazos críticos.
argument-hint: "[describe el caso o pega documentos disponibles]"
---
...
1. **`mcp__robin__analizar_expediente`** con los documentos/hechos.

2. **`mcp__robin__cronologia_hechos`** para línea temporal.

3. **`mcp__robin__mapa_documental`** para identificar documentos
   clave y huecos probatorios.

4. **Foral-check**.

5. **`mcp__robin__estimar_viabilidad_caso`**: probabilidad por
   escenario (mejor / esperable / peor) con razonamiento.

6. **`/robin:plazos`** para prescripciones, caducidades, plazos
   procesales activos.

7. **`mcp__robin__generar_plan_estrategico`** con hoja de ruta
   priorizada en 5-8 hitos.

8. **Verifica TODAS las citas** que aparezcan en el análisis.
...
Cronología
[línea temporal con hitos]

Mapa documental
- Documentos clave (qué hay / qué falta / cómo obtener lo que falta)
- Pruebas pendientes (testifical, pericial, documental complementaria)

Viabilidad
- Escenario favorable (probabilidad %): […]
- Escenario esperable (probabilidad %): […]
- Escenario adverso (probabilidad %): […]

Plazos críticos
[fecha + tipo + consecuencia]
...
Coste estimado para el cliente
- Tasas: […]
- Costas potenciales: […]
- Honorarios: a determinar por el despacho según política
...
El resultado se guarda en `matter.md` del matter activo. El agent
`vigilancia-plazos` lo lee para alertar de fechas críticas.
