...
name: cliente-email
description: >
  Email al cliente: actualización de caso, explicación técnica adaptada,
  solicitud de información, comunicación de resolución. Tono y registro
  según playbook.
argument-hint: "[propósito + cliente + información a comunicar]"
---
...
1. **`mcp__robin__preparar_email_cliente`** con propósito + datos
   contextuales del matter si lo hay.

2. **Adaptación tono**: leer playbook (sobrio / conciliador / asertivo;
   técnico vs. accesible).

3. **Estructura**:
   - Saludo (formal o cercano según relación).
   - Contexto (1-2 frases).
   - Asunto principal (claro, en bullets si es información compleja).
   - Próximos pasos (qué necesitas del cliente / qué hará el despacho).
   - Cierre con disponibilidad.

4. **Lenguaje**: técnico solo lo imprescindible; cada término técnico
   con paréntesis explicativo o glosa.

5. **Confidencialidad**: pie con aviso si el email viaja por
   canales no seguros.

6. **Verifica citas** si el email cita normas o sentencias.

## Avisos típicos

- Privilegio abogado-cliente: usar canal cifrado siempre que se
  pueda; recordar al cliente reenviar a destinatarios autorizados.
- Si el email incluye datos personales sensibles: cifrado obligatorio.
