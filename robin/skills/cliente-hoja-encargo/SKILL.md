---
name: cliente-hoja-encargo
description: >
  Hoja de encargo profesional al cliente: ámbito del trabajo, honorarios,
  provisiones, condiciones de terminación, política de costas, RGPD.
argument-hint: "[cliente + asunto + ámbito del encargo + honorarios]"
---

# /robin:cliente-hoja-encargo

Pipeline:

1. **`mcp__robin__preparar_hoja_encargo`** con datos del despacho +
   cliente + asunto.

2. **Cláusulas obligatorias o muy recomendables**:
   - Identificación del despacho y del letrado responsable.
   - Identificación del cliente.
   - Objeto del encargo (delimitado).
   - Honorarios: fija / por horas / mixta / éxito (legalidad de
     pacto de cuota litis vs. pacto de éxito).
   - Provisiones de fondos.
   - Suplidos: facturación separada.
   - Costas a favor / contra el cliente.
   - Terminación del encargo (causas, plazo, devolución de fondos).
   - Régimen de comunicaciones.
   - RGPD: información del tratamiento de datos del cliente.
   - Limitación de responsabilidad y aseguramiento RC.
   - Ley aplicable y mediación / arbitraje.

3. **Política colegial**: ajustar a los criterios orientadores del
   ICAM/colegio donde esté el despacho.

4. **Si el cliente es consumidor**: cumplimiento normativa
   consumidores (TRLGDCU). Documento contractual claro,
   transparencia, posibilidad de revocación si servicios a distancia.

5. **Verifica citas**.

## Avisos típicos

- Pacto de cuota litis: STS y CGAE han marizado su admisibilidad.
  Compatible con éxito + porcentaje de costas.
- Cláusula de no competencia (cliente no contrata otro despacho
  para el mismo asunto): vigilar antitrust + libertad del cliente.
