...
name: contrato-nda
description: >
  Acuerdo de confidencialidad (NDA). Unilateral o mutuo. Calibrado a la
  posición de quien revela vs. quien recibe información.
argument-hint: "[partes + propósito + información objeto + lado representado]"
---
...
1. **Tipo de NDA**:
   - Unilateral (un único revelador).
   - Mutuo (ambas partes revelan).

2. **`mcp__robin__preparar_redaccion_contrato`** con tipo=nda.

3. **Bloques estándar**:
   - Definición de "información confidencial" (suficientemente amplia
     vs. acotada según lado).
   - Excepciones (información pública, obtenida lícitamente, ya
     conocida, requerida por ley).
   - Obligaciones del receptor (uso limitado, medidas de seguridad,
     personal autorizado, devolución/destrucción al término).
   - Duración del deber (típico 3-5 años post-terminación; sin
     límite para secretos comerciales si encajan en Ley 1/2019 de
     secretos empresariales).
   - Penalización por incumplimiento.
   - Ley y jurisdicción.

4. **Defensiva si receptor**: limitar duración, definir confidencial
   solo lo marcado expresamente, excluir información ya conocida.

5. **Ofensiva si revelador**: amplitud, duración larga, penalización
   alta, jurisdicción favorable.

6. **Cláusula adicional si encaja**: secretos empresariales (Ley
   1/2019) — refuerzo de tutela y acciones específicas.

7. **Verifica citas**.

## Avisos típicos

- NDA con extranjero UE: ley elegida + jurisdicción. Considerar
  arbitraje en Madrid si conviene neutralidad.
- Si la información incluye datos personales: NDA no exime de
  cumplir RGPD; necesario DPA específico.
- Cláusula penal: el TS la modera si desproporcionada (art. 1154 CC);
  fijar importe razonable.
