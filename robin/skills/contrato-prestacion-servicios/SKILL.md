---
name: contrato-prestacion-servicios
description: >
  Contrato de prestación de servicios (consultoría, IT, profesionales).
  Distinción con laboral, niveles de servicio (SLA), propiedad de entregables.
argument-hint: "[servicios + partes + duración + lado representado]"
---

# /robin:contrato-prestacion-servicios

Pipeline:

1. **Test laboral**. Riesgo crítico si los indicios apuntan a
   relación laboral encubierta (dependencia, ajenidad, retribución
   continua, horarios). Si concurren, advertir y, si el cliente
   insiste, calibrar redacción para minimizar.

2. **`mcp__robin__preparar_redaccion_contrato`** con
   tipo=prestacion_servicios.

3. **Bloques específicos**:
   - Descripción de los servicios (SOW específico o anexo).
   - SLA (niveles de servicio): disponibilidad, tiempos de respuesta,
     penalizaciones.
   - Equipo asignado.
   - Plazo y entregables intermedios.
   - Aceptación de entregables (criterios + plazo + silencio
     positivo/negativo).
   - Precio: tarifa por hora / proyecto cerrado / mixta.
   - Pago y morosidad (Ley 3/2004).
   - Propiedad intelectual de los entregables (cesión vs. licencia).
   - Subcontratación: permitida o no.
   - Personal clave y rotaciones.
   - Confidencialidad y datos.
   - Limitación de responsabilidad.
   - Terminación: anticipada, por causa, conveniencia, efectos.

4. **Si el servicio implica tratamiento de datos personales**: DPA
   conexa obligatoria (→ `/robin:rgpd-dpa`).

5. **Verifica citas**.

## Avisos típicos

- Servicios IT con SLA estrictos: cláusula de fuerza mayor / hechos
  exonerantes.
- Cesión de IP: art. 17 LPI (obras de software por encargo) y arts.
  43-44 LPI (cesión expresa).
- Si hay falsos autónomos: riesgo de ITSS + TGSS por presunción
  laboral (LRJS).
