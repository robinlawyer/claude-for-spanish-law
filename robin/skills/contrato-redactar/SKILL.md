---
name: contrato-redactar
description: >
  Redacción de contrato a medida desde cero. Cláusulas estándar + cláusulas
  específicas según operación. Personalizado a la posición de la parte que
  representas.
argument-hint: "[tipo de contrato + partes + perímetro + lado representado]"
---

# /robin:contrato-redactar

Pipeline:

1. **Identifica el tipo**. Si encaja con una skill especializada
   (NDA, prestación servicios, distribución, agencia, franquicia,
   suministro), deriva a esa skill específica.

2. **`mcp__robin__preparar_redaccion_contrato`** con tipo + partes +
   datos esenciales.

3. **Foral-check** si procede.

4. **Cláusulas tipo**:
   - Identificación de partes y poderes.
   - Objeto y alcance.
   - Precio y pago.
   - Plazo y prórroga.
   ...
   - Protección de datos (cláusula RGPD + remisión a DPA si procede).
   - Limitación de responsabilidad.
   - Penalizaciones.
   - Terminación (causas + procedimiento).
   - Ley aplicable y jurisdicción / arbitraje.
   - Cláusulas finales (integridad, modificación por escrito,
     comunicaciones).

5. **Cláusulas específicas según operación**: ajustar al sector y
   posición.

6. **Verifica citas**.

7. **Documento entregable** en formato estándar de oficina.

## Avisos típicos

- Si interviene Administración Pública: deriva a /robin:admin-contratacion-publica.
- Si es laboral: deriva a skills laborales (este flujo es para
  mercantil).
- Si las partes ofrecen pre-borrador: deriva a /robin:contrato-revisar.
