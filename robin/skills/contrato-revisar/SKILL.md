---
name: contrato-revisar
description: >
  Revisión de contrato contra ley imperativa, doctrina y mejores prácticas.
  Identifica cláusulas problemáticas, propone redactados alternativos, evalúa
  riesgos para la parte que representas.
argument-hint: "[contrato a revisar + lado que defendemos]"
---
...
1. **`mcp__robin__preparar_revision_contrato`** con el texto del
   contrato + lado (comprador/vendedor, prestador/cliente,
   licenciante/licenciatario, arrendador/arrendatario, etc.).

2. **Identificación del tipo**. Es esencial: las reglas imperativas
   cambian por tipo (compraventa CCom/CC, prestación servicios,
   arrendamiento LAU vs CC, distribución, agencia LCA, franquicia,
   etc.).

3. **Foral-check** si el caso tiene punto de conexión foral
   relevante en obligaciones y contratos (Llibre Sisè CCCat,
   compilaciones forales).

4. **Auditor de cláusulas problemáticas**:
   - Limitación de responsabilidad excesiva (control art. 1102 CC:
     dolo no excluible; doctrina TS sobre exoneración por
     negligencia grave).
   - Cláusulas abusivas si hay consumidor (TRLGDCU).
   - Sometimiento a fuero contrario a normativa imperativa.
   - Ley aplicable contraria a normas internacionalmente imperativas.
   - Penalizaciones desproporcionadas (art. 1154 CC moderación).
   - Plazos de pago contrarios a Ley 3/2004 morosidad.
   - Cláusulas de no competencia abusivas (proporcionalidad).
   - Indemnidades excesivas o ilegales.

5. **Jurisprudencia sobre cláusulas controvertidas**.
   `/robin:jurisprudencia` con `--norma <referencia>` sobre la
   cláusula. Aplica jerarquía.

6. **Redacción alternativa**: para cada cláusula problemática,
   propón redacción mejorada.

7. **Verifica TODAS las citas**.

## Formato

```
📋 Revisión de contrato — [tipo]

Resumen ejecutivo
[3-5 líneas: estado general, riesgos críticos, recomendación firma]

Cláusulas problemáticas
🔴 CRÍTICAS (no firmar sin modificar)
1. Cláusula X — Riesgo: […]
   Redacción actual: "[…]"
   Redacción propuesta: "[…]"
   Base jurídica: [norma + ECLI]

🟠 IMPORTANTES (negociar)
1. […]

🟡 MEJORABLES (si la otra parte cede)
1. […]

Comparación con el sector
[Si tienes política de despacho sobre este tipo de contrato, contraste]

Cláusulas que faltan
[A añadir]

Avisos al letrado
- Si hay consumidor: control de oficio de abusividad por jueces.
- Si es UE transfronterizo: Roma I y Bruselas Ibis aplicables.
- Si afecta a datos personales: DPA conexa obligatoria (→ /robin:rgpd-dpa).
```

## Avisos típicos

- Cláusulas de jurisdicción exclusiva en contratos con consumidor:
  no válidas si imponen domicilio que no es del consumidor (TJUE
  *Pannon*, *Aziz*).
- Cláusulas de no competencia post-contractual: máximo 2 años (CC y
  doctrina), proporcional al perjuicio, retribuida.
