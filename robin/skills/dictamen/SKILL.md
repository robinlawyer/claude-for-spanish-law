...
name: dictamen
description: >
  Dictamen jurídico formal. Análisis técnico exhaustivo sobre una cuestión
  concreta con conclusión motivada y verificación de citas.
argument-hint: "[cuestión jurídica + contexto]"
---
...
1. **`mcp__robin__preparar_dictamen`** con la cuestión.

2. **Foral-check** si materia civil/mercantil/familia/sucesoria/
   inmobiliaria.

3. **Marco normativo**: `mcp__robin__obtener_articulo_ley` sobre
   preceptos centrales. Cita literal si decisivo.

4. **Jurisprudencia jerárquica** vía `/robin:jurisprudencia`.

5. **Doctrina vinculante** si procede: TC, TJUE, TEDH, AEPD, DGT,
   TEAC, Consejo de Estado.

6. **Análisis**: confrontar interpretaciones, identificar la más
   sólida y las alternativas.

7. **Conclusión motivada**: posición jurídica más sólida + nivel de
   certeza + riesgos.
...
1. **Antecedentes** (resumen de hechos y cuestión planteada).
2. **Cuestiones jurídicas a resolver** (numeradas).
3. **Marco normativo** (textos literales de los preceptos clave).
4. **Análisis** (jurisprudencia + doctrina + lógica jurídica).
5. **Conclusión** (clara, en lenguaje accesible al cliente
   destinatario).
6. **Anexo de citas verificadas**.
