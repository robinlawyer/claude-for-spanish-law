---
name: familia-modificacion-medidas
description: >
  Modificación de medidas definitivas adoptadas en sentencia de divorcio o
  separación (alimentos, custodia, vivienda) por alteración sustancial de
  circunstancias.
argument-hint: "[medida vigente + cambio de circunstancias]"
---

# /robin:familia-modificacion-medidas

Pipeline:

1. **Alteración sustancial de circunstancias** (art. 90.3 CC):
   imprevisible, no causada voluntariamente por el solicitante,
   actual y mantenida.

2. **`mcp__robin__preparar_escrito_familia`** con tipo=modificacion_medidas.

3. **Procedimiento**: el del art. 775 LEC.

4. **Si los cónyuges están de acuerdo**: trámite simplificado por
   convenio.

5. **Doctrina TS y AAPP del territorio**.

6. **Verifica citas**.

## Avisos típicos

- Pérdida de empleo: por sí sola no basta, hay que acreditar
  involuntariedad + búsqueda activa de empleo.
- Nuevas familias: la formación de una nueva familia no extingue
  automáticamente alimentos a hijos previos.
