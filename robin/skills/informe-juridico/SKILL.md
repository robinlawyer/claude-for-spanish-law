---
name: informe-juridico
description: >
  Informe jurídico formal para cliente no abogado. Lenguaje accesible,
  estructura clara: situación, marco legal, opciones, recomendación, plan
  de acción.
argument-hint: "[problema o consulta del cliente]"
---

# /robin:informe-juridico

Pipeline:

1. **`mcp__robin__preparar_informe_juridico`** con el problema.

2. **Análisis técnico** (interno, no se vuelca al cliente):
   `/robin:jurisprudencia` + `mcp__robin__obtener_articulo_ley` +
   doctrina específica.

3. **Estimación económica**: `mcp__robin__tasa_judicial`,
   `mcp__robin__calcular_costas_procesales`, intereses, etc.

4. **Plazos**: `/robin:plazos`.

5. **Viabilidad**: `mcp__robin__estimar_viabilidad_caso`.

6. **Verifica TODAS las citas** (van en anexo técnico).

## Formato

Para el cliente (lenguaje claro):

1. **Resumen ejecutivo** (3-5 líneas).
2. **Situación de partida**.
3. **Marco legal aplicable** (explicación accesible).
4. **Análisis** (qué dice la jurisprudencia, escenarios).
5. **Opciones disponibles** (pros / contras de cada una).
6. **Recomendación**.
7. **Plan de acción** (pasos, plazos, costes estimados).

Anexo técnico para abogados:
- Citas con ECLI verificado.
- Razonamiento jurídico detallado.
- Riesgos identificados.

## Avisos típicos

- Tono al cliente: profesional sin condescender. Explica conceptos
  técnicos sin sacrificar precisión jurídica.
- Si hay incertidumbre: dilo. Mejor avisar al cliente que vender una
  certeza falsa.
