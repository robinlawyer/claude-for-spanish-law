---
name: ip-registro-patente
description: >
  Solicitud de patente nacional, europea (EPO) o internacional (PCT).
  Memoria, reivindicaciones, búsqueda del estado de la técnica.
argument-hint: "[invención + ámbito + estado del arte conocido]"
---

# /robin:ip-registro-patente

Pipeline:

1. **Requisitos de patentabilidad**: novedad, actividad inventiva,
   aplicación industrial. Materias excluidas (descubrimientos
   científicos, métodos matemáticos, software puro sin efecto técnico,
   métodos de tratamiento, etc.).

2. **Estrategia de protección**:
   - Patente española (OEPM): cobertura nacional.
   - Patente europea (EPO): validable en estados elegidos.
   - PCT: solicitud internacional, fases nacionales posteriores.
   - Modelo de utilidad: requisitos menos exigentes, vida más corta
     (10 años).

3. **Memoria técnica + reivindicaciones**: redacción crítica.

4. **`mcp__robin__preparar_solicitud_pi`** con tipo=patente.

5. **Tiempos y costes**: OEPM ~3 años con examen; EPO 3-5 años;
   PCT facilita estrategia pero coste.

6. **Doctrina EPO Boards of Appeal + TS Sala 1ª** sobre nulidad y
   alcance.
...
- Software con efecto técnico: patentable como invención implementada
  por ordenador (EPO).
- Convenios de empleados: titularidad del inventor empleado (art.
  15 LP).
