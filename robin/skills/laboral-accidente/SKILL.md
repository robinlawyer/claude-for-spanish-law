---
name: laboral-accidente
description: >
  Accidente de trabajo o enfermedad profesional. Acción de recargo de
  prestaciones (art. 164 LGSS), reclamación civil concurrente, gestión con
  Mutua.
argument-hint: "[descripción del accidente + lesiones + medidas preventivas]"
---

# /robin:laboral-accidente

Pipeline:

1. **Calificación**: accidente de trabajo (art. 156 LGSS) /
   enfermedad profesional / enfermedad común.

2. **Investigación**: parte de accidente (modelo Delt@), informe
   ITSS si hay falta de medidas preventivas.

3. **Acciones del trabajador**:
   - Prestaciones SS: IT, IP, viudedad si fallecimiento.
     `mcp__robin__calcular_prestacion_seguridad_social`.
   - Recargo de prestaciones (30-50 %) si falta de medidas
     preventivas (art. 164 LGSS).
   - Indemnización civil concurrente (art. 1902 CC + arts.
     consolidados sobre concurrencia con SS).
   - Acción penal si dolo o grave imprudencia (arts. 316-318 CP).

4. **`mcp__robin__preparar_supuesto`** con tipo=accidente_trabajo.

5. **Doctrina TS Sala 4ª sobre recargo**:
   `/robin:jurisprudencia --norma "art. 164 LGSS"`.

6. **Verifica citas**.

## Avisos típicos

- Indemnización por accidente: se aplica baremo de tráfico de forma
  orientativa (`mcp__robin__calcular_baremo_trafico`) en sede social
  por doctrina TS reiterada.
- ITSS levanta acta de infracción + propuesta de recargo: doble vía
