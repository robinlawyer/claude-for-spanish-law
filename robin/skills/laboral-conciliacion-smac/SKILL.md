---
name: laboral-conciliacion-smac
description: >
  Papeleta de conciliación previa ante el SMAC (UMAC en algunas CCAA), trámite
  obligatorio antes de demanda social (art. 63 LRJS).
argument-hint: "[tipo de pretensión + datos del trabajador y empresa]"
---

# /robin:laboral-conciliacion-smac

Pipeline:

1. **Verificar obligatoriedad**. Excepciones del art. 64 LRJS
   (vacaciones, materia electoral, conflicto colectivo, impugnación
   convenio, derechos fundamentales).

2. **`mcp__robin__preparar_supuesto`** con tipo=papeleta_smac.

3. **Datos esenciales**:
   - Identificación demandante y demandada (empresa + grupo
     empresarial si aplica + Administración titular si pública).
   - Hechos sucintos.
   - Pretensión concreta y cuantificada.

4. **Presentación**: SMAC del lugar de prestación o domicilio del
   demandante. Plazo: interrumpe la prescripción/caducidad.

5. **Trámite**: citación a comparecencia (puede ser ≤ 30 días).
   Posibles resultados: avenencia, sin avenencia, sin efecto (no
   comparece la demandada).

6. **Verifica citas**.

## Avisos típicos

- No presentar SMAC cuando es obligatorio: archivo de la demanda
  por defecto procesal.
- Avenencia: título ejecutivo (art. 68 LRJS).
- Si la empresa no comparece: certificación "sin efecto" sirve igual.
