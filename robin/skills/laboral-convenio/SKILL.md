---
name: laboral-convenio
description: >
  Identificación del convenio colectivo aplicable y lectura asistida de
  cláusulas relevantes para un caso concreto.
argument-hint: "[sector + provincia + actividad + cuestión consultada]"
---

# /robin:laboral-convenio

Pipeline:

1. **`mcp__robin__buscar_convenio_colectivo`** con sector +
   provincia + actividad. Devuelve convenio aplicable + ámbito
   temporal + cláusulas indexadas.

2. **Concurrencia de convenios** (art. 84 ET): si hay convenio
   sectorial estatal + sectorial autonómico + de empresa, regla de
   prevalencia.

3. **Cláusula consultada**: localizar literalmente.

4. **Jurisprudencia interpretativa**: `/robin:jurisprudencia
   --organo TS --sala social` sobre la cláusula.

5. **Si convenio caducado**: ultraactividad (1 año desde
   denuncia + cláusula de mantenimiento del convenio o normativa
   subsidiaria).
...
- Convenios autonómicos pueden mejorar el estatal pero no en
  materias de competencia exclusiva estatal (art. 84.2 ET).
- Si descuelgue (art. 82.3 ET): empresa puede inaplicar tablas
  salariales por causas económicas con procedimiento.
