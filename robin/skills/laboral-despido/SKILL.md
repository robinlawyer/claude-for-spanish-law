...
description: >
  Revisión de despido (objetivo, disciplinario, improcedente, nulo). Cálculo
  de indemnización, plazos y procedencia. Preparación de papeleta SMAC y
  demanda social.
argument-hint: "[tipo de despido + datos del trabajador + carta de despido]"
---
...
1. **Tipificar** (art. 49 + 52-56 ET): objetivo, disciplinario,
   colectivo, fin de contrato. Comprobar requisitos formales de la
   carta (causa concreta, hechos con fecha, identificación de
   normativa).

2. **Plazo de impugnación**. `/robin:plazos` — 20 días hábiles
   sociales desde notificación (art. 59.3 ET).

3. **Convenio aplicable**. `mcp__robin__buscar_convenio_colectivo`
   con sector + provincia + actividad.

4. **`mcp__robin__preparar_supuesto`** con tipo=despido.

5. **Calcular indemnización**.
   `mcp__robin__calcular_indemnizacion_despido`:
   - Improcedente: 33 días/año (post 12/02/2012; 45 días previos).
   - Objetivo procedente: 20 días/año, máx 12 mensualidades.
   - Salario regulador con prorrateo de pagas + variables últimos
     12 meses.

6. **Jurisprudencia social TS Sala 4ª**. `/robin:jurisprudencia
   --organo TS --sala social`. Doctrina sobre causa, forma,
   salario regulador.

7. **Papeleta de conciliación**. `/robin:laboral-conciliacion-smac`.

8. **Demanda social**. `/robin:laboral-demanda-social`.

9. **Verifica citas**.

## Avisos típicos

- Despido por causa relacionada con derechos fundamentales: NULO
  (art. 55.5 ET). Readmisión obligatoria + salarios de tramitación
  + indemnización daños y perjuicios.
- Embarazada, lactancia, conciliación: nulidad cualificada salvo
  causa ajena.
- Si hay representación legal: garantías de los arts. 68 y 56 ET.
- Despido durante IT: válido si causa real distinta de la IT
  (doctrina TS reciente sobre discriminación por enfermedad).
