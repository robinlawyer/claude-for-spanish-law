---
name: mercantil-financiacion
description: >
  Documentación de operaciones de financiación: contrato de préstamo
  mercantil, sindicado, mezzanine, crédito revolving, garantías reales y
  personales, term sheet, covenants.
argument-hint: "[tipo de financiación + prestamista/prestatario + estructura + garantías]"
---

# /robin:mercantil-financiacion

Pipeline:

1. **Tipo de financiación**:
   - Préstamo bilateral / sindicado.
   - Crédito revolving / línea.
   - Préstamo subordinado / mezzanine.
   - Bono o pagaré.
   - Project finance.

2. **`mcp__robin__preparar_documento_financiacion`** con tipo + estructura
   + partes.

3. **Documentos típicos**:
   - **Term sheet** vinculante o no.
   - **Facility agreement** (LMA-style si internacional).
   - **Garantías**: hipoteca, prenda, aval, garantía solidaria.
   - **Subordinación** entre acreedores (intercreditor agreement).
   - **Información financiera** y MAC clauses.

4. **Covenants** (financieros + de información + de conducta) ajustados
   al sector y tamaño.

5. **Implicaciones regulatorias**:
   - Reserva de actividad de entidades de crédito (LOSS) si presta
     entidad no bancaria.
   - Información al cliente bancario (BdE) si consumidor.
   - Mercados de valores (CNMV) si bono o estructura.
   - Doctrina BdE relevante: `mcp__robin__buscar_bde`.

6. **Implicaciones fiscales**: retenciones IRPF/IRNR sobre intereses,
   ITPyAJD sobre hipoteca, IVA sobre comisiones.

7. **Verifica citas**.

## Avisos típicos

- Cláusulas suelo, IRPH y gastos hipotecarios en hipotecas con
  consumidor: doctrina TJUE consolidada y TS reciente. Riesgo de
  nulidad parcial.
- Garantía a primer requerimiento vs. fianza: TS distingue, redacción
  precisa imprescindible.
- Operaciones transfronterizas: ley aplicable (Roma I), jurisdicción
  (Bruselas Ibis), sumisión a arbitraje admisible.
