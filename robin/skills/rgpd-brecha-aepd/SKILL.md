---
name: rgpd-brecha-aepd
description: >
  Notificación de brecha de seguridad a la AEPD en 72 horas (art. 33 RGPD) y,
  si procede, comunicación a los interesados (art. 34).
argument-hint: "[descripción de la brecha + número de afectados + categorías de datos]"
---

# /robin:rgpd-brecha-aepd

Pipeline:

1. **Evaluación de la brecha**:
...
2. **¿Es notificable a AEPD?** Sí, salvo que sea improbable que
   suponga riesgo para los derechos y libertades. Default: notificar.

3. **¿Es notificable a interesados?** Sí si riesgo alto (probabilidad
   + impacto). Default si la brecha incluye contraseñas, datos
   bancarios, datos sensibles, identificadores únicos.

4. **Plazo notificación AEPD**: 72 h desde tener conocimiento.
   Notificaciones por fases si la información no está completa.

5. **`mcp__robin__preparar_escrito_libre`** con tipo=brecha_aepd.

6. **Comunicación a interesados** si procede: lenguaje claro, sin
   tecnicismos. Información mínima: naturaleza, datos de contacto
   DPO, consecuencias probables, medidas adoptadas, recomendaciones
   al interesado.

7. **Registro interno** de la brecha (art. 33.5): obligatorio.

8. **Doctrina AEPD** sobre brechas análogas:
...
Dos documentos:
1. Notificación AEPD (canal oficial sede.aepd.gob.es).
2. Comunicación a interesados (si procede).

## Avisos típicos

- Brecha en encargado: notifica al responsable, no directamente a
  AEPD (salvo doble rol).
- Si la brecha afecta a sector regulado (financiero, sanitario,
  telecos), notificaciones adicionales a CNMC, BdE, CNMV, etc.
- Riesgo de sanción si se descubre que la brecha no se notificó en
  plazo. Cuantía AEPD: variable, doctrina abundante.
