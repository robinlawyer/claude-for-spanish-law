...
name: rgpd-cookies
description: >
  Banner y política de cookies conforme Guía AEPD 2023 y LSSI art. 22.2.
  Consentimiento granular previo al primer uso de cookies no exentas.
argument-hint: "[web + cookies utilizadas + finalidades]"
---
...
1. **Inventario de cookies**: técnicas / analíticas / publicidad /
   personalización / terceros. Si el cliente no lo tiene, ayudarle a
   construirlo (escaneo + verificación).

2. **Calificación**: exentas (estrictamente necesarias) vs. sujetas
   a consentimiento.

3. **Banner conforme Guía AEPD 2023**:
   - Capa 1: rechazar igual de fácil que aceptar; sin patrones
     oscuros.
   - Sin cookies no exentas hasta consentimiento.
   - Información clara sobre finalidad y responsables (especialmente
     terceros).
   - Configuración granular obligatoria.

4. **Política de cookies completa** (capa 2): listado, finalidad,
   plazo de conservación, terceros, instrucciones para desactivar
   en el navegador.

5. **`mcp__robin__preparar_dictamen`** con tipo=cookies_aepd.

6. **Doctrina AEPD sancionadora sobre cookies**:
   `mcp__robin__buscar_aepd` con `cookies` — abundante.

7. **Si hay transferencia internacional** vía cookies de terceros
   (Google Analytics, Meta, etc.): cobertura adicional.
...
Entregables:
- Texto del banner (capa 1).
- Texto de la política de cookies (capa 2).
- Instrucciones técnicas mínimas para el equipo desarrollador.
