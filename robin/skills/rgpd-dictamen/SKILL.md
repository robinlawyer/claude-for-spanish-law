...
name: rgpd-dictamen
description: >
  Dictamen jurídico de cumplimiento RGPD/LOPDGDD para un tratamiento concreto.
  Cruza con doctrina AEPD por expediente, jurisprudencia TJUE y normativa UE
  (RGPD + AI Act + ePrivacy).
argument-hint: "[descripción del tratamiento + responsable + datos + finalidad + transferencias]"
---
...
1. **`mcp__robin__auditar_compliance_rgpd`** con los datos del
   tratamiento. Devuelve checklist por principio.

2. **Doctrina AEPD vinculante**. `mcp__robin__buscar_aepd` con la
   finalidad/sector. Citar expediente AEPD por número y resumen
   resolución.

3. **Jurisprudencia TJUE**. `mcp__robin__buscar_tjue` sobre el
   supuesto (consentimiento, interés legítimo, decisiones
   automatizadas, transferencias, derecho al olvido, etc.).

4. **Si hay IA o decisiones automatizadas significativas**: añadir
   Reglamento UE 2024/1689 (AI Act) y art. 22 RGPD.

5. **Sanción potencial**: identificar banda del art. 83 RGPD (10/20
   M€ o 2/4 % facturación mundial).

6. **Verifica todas las citas**.

## Formato

```
📊 Dictamen RGPD — [tratamiento]

Antecedentes
[Resumen del tratamiento + responsable + datos]
...
▶ Información al interesado (arts. 13-14)
[…]
...
▶ EIPD (art. 35) — necesaria?
[…]

▶ Transferencias internacionales (Cap. V)
[…]

▶ Brechas de seguridad (arts. 33-34)
[…]

Riesgo de sanción
- Banda art. 83.4: …
- Banda art. 83.5: …
- Antecedentes del responsable: [si los hay en AEPD]

Recomendaciones priorizadas
🔴 Críticas (implementar antes de seguir tratando):
🟠 Altas:
🟡 Medias:

Anexo: citas verificadas
[tabla]
```

## Avisos típicos

- Si el tratamiento implica datos de menores: art. 8 RGPD + art. 7
  LOPDGDD (consentimiento del titular de la patria potestad < 14
  años).
- Si hay categorías especiales (art. 9): excepción específica
  obligatoria.
- Brechas: notificación a AEPD en 72 h (art. 33), a interesados si
  riesgo alto (art. 34).
