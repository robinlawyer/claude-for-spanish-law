...
name: rgpd-pia
description: >
  Evaluación de impacto en protección de datos (EIPD/PIA) conforme art. 35
  RGPD y guía AEPD. Identifica riesgos y medidas mitigadoras.
argument-hint: "[tratamiento que requiere EIPD]"
---
...
1. **¿Es obligatoria la EIPD?** Triaje:
   - Lista de tratamientos AEPD que sí requieren (decisión
     automatizada con efecto jurídico, datos sensibles a gran escala,
     observación sistemática zonas públicas, etc.).
   - Doctrina AEPD: `mcp__robin__buscar_aepd` para casos análogos.

2. **`mcp__robin__auditar_compliance_rgpd`** con modo EIPD.

3. **Estructura de la EIPD**:
...
4. **Tipología de riesgos**:
   - Acceso no autorizado, modificación, pérdida.
   - Tratamiento más allá de la finalidad.
   - Falta de exactitud, conservación excesiva.
   - Decisiones discriminatorias o erróneas.
   - Pérdida de control del interesado.

5. **Medidas mitigadoras**: técnicas (cifrado, pseudonimización,
   control de acceso) y organizativas (políticas, formación,
   contratos, DPA).

6. **Consulta previa a la AEPD** (art. 36 RGPD): si EIPD muestra
   riesgo residual alto. Procedimiento.

7. **Verifica citas**.

## Formato

EIPD completa lista para presentar a la AEPD si procede consulta
previa.
