---
name: rgpd-transferencias
description: >
  Transferencias internacionales de datos personales fuera del EEE.
  Mecanismos del Capítulo V RGPD: decisión de adecuación, SCC, BCR, etc.
argument-hint: "[país destino + datos + finalidad + mecanismo previsto]"
---

# /robin:rgpd-transferencias

Pipeline:

1. **País destino**. Verificar si tiene decisión de adecuación de
   la Comisión UE vigente (Reino Unido, Suiza, Israel, Canadá
   comercial, Japón, Corea del Sur, USA con marco DPF para
   organizaciones adheridas, etc.).

2. **Si hay adecuación**: transferencia libre con cumplimiento
   resto RGPD.

3. **Si no hay adecuación**: mecanismo del art. 46:
   - SCC (cláusulas tipo Comisión 2021/914).
   - BCR (binding corporate rules) — multinacional.
   - Códigos de conducta certificados.
   - Mecanismos de certificación.
   - Cláusulas contractuales ad hoc autorizadas por AEPD.

4. **Transfer Impact Assessment (TIA)** post Schrems II
   (STJUE C-311/18): evaluación del derecho del país destino,
   medidas suplementarias si las garantías de SCC no son
   suficientes.

5. **Excepciones para situaciones específicas** (art. 49):
   consentimiento explícito, ejecución de contrato, interés
   público, etc. — solo casos puntuales, no flujo continuo.

6. **`mcp__robin__preparar_dictamen`** con tipo=transferencia_internacional.

7. **Doctrina AEPD y TJUE**: `mcp__robin__buscar_aepd` +
   `mcp__robin__buscar_tjue`.

8. **Verifica citas**.

## Formato

Dictamen + plantilla SCC con anexos rellenos + TIA si procede.

## Avisos típicos

- USA: si la organización destinataria no está adherida a DPF,
  necesidad de SCC + TIA + medidas suplementarias.
- China, Rusia: TIA siempre con alto riesgo; medidas suplementarias
  fuertes (cifrado extremo a extremo, pseudonimización).
- Reagrupación de datos en gestor de TI internacional: subencargado
  + DPA + transferencia.
