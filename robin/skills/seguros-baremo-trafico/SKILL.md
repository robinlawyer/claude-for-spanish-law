---
name: seguros-baremo-trafico
description: >
  Cálculo de indemnización por accidente de tráfico conforme baremo (Ley
  35/2015) con actualización IPC automática.
argument-hint: "[fecha accidente + lesiones temporales + secuelas + perjuicio moral]"
---

# /robin:seguros-baremo-trafico

Pipeline:

1. **Fecha del accidente** (determina versión del baremo + IPC).

2. **`mcp__robin__calcular_baremo_trafico`** con:
   - **Lesiones temporales**: días impeditivos / no impeditivos /
     hospitalarios + intervenciones.
   - **Secuelas**: tipo + puntos (Anexo III baremo).
   - **Perjuicio moral por pérdida de calidad de vida** (3 grados:
     leve / moderado / grave + perjuicio personal particular).
   - **Lucro cesante** (corrección incapacidad para trabajo).
   - **Pérdida calidad de vida de familiares** (perjudicados
     ampliados).
   - **Fallecimiento**: pretium doloris para familiares + lucro
     cesante.

3. **Aplicación analógica fuera de tráfico**: doctrina TS reciente
   permite usar baremo orientativamente en accidentes laborales y
...
Tabla por concepto + total + base normativa para cada partida.

## Avisos típicos

- Edad de la víctima modifica el cálculo (especialmente lucro
  cesante).
- Tablas se actualizan anualmente con IPC: Robin auto-update.
