---
name: societario-acuerdo-junta
description: >
  Convocatoria, orden del día, certificación y elevación a público de
  acuerdos sociales (junta general / consejo de administración).
argument-hint: "[tipo de acuerdo + sociedad + orden del día]"
---

# /robin:societario-acuerdo-junta

Pipeline:

1. **Verifica órgano competente** (LSC). Capital, modificaciones
   estructurales, retribución administradores → junta. Gestión y
   poderes → consejo.

2. **Convocatoria**:
   - Forma (BORME + diario / web corporativa registrada / comunicación
     individual si estatutos lo permiten).
   - Plazo (1 mes para SA ordinaria; 15 días para SL).
   - Contenido del orden del día.

3. **`mcp__robin__preparar_acuerdo_societario`** con tipo y materia.

4. **Quórum y mayorías**:
   - Generales (LSC art. 198-202).
   - Reforzadas si estatutos o materia lo exigen (modificación
     estatutos, transformación, fusión, escisión).

5. **Acta**: estructura del acta, firmas, depósito (si procede).

6. **Certificación + elevación a público + inscripción**: pasos
...
- Borrador de escritura de elevación a público

## Avisos típicos

- Si hay socios extranjeros que requieren consulta vinculante DGT
  fiscal, considerar implicaciones IRNR.
- Modificaciones estructurales requieren informe del administrador
  y, si SA, además de los expertos independientes.
