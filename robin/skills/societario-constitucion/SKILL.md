---
name: societario-constitucion
description: >
  Constitución de SL o SA: capital, socios, estatutos, objeto, denominación,
  domicilio, órgano de administración, aportaciones.
argument-hint: "[tipo SL/SA + socios + objeto + capital]"
---
...
1. **Tipo societario**. SL (capital mínimo 3.000 €, transmisión
   restringida) vs. SA (60.000 €, transmisión libre por defecto).
   Considerar SLU si socio único.

2. **Denominación**:
   - Certificación negativa del Registro Mercantil Central (CSC).

3. **Capital y aportaciones**:
   - Dinerarias (depósito en cuenta a nombre de la sociedad en
     constitución).
   - No dinerarias (descripción + valoración; si SA, informe de
     experto independiente).

4. **Estatutos**. Generar a partir de los 33 modelos mercantiles
   oficiales del corpus Robin (`mcp__robin__preparar_acuerdo_societario`
   con tipo=estatutos).
...
5. **Órgano de administración**:
   ...
   - Consejo de administración (mínimo 3 SA, sin mínimo SL salvo
     estatutos).

6. **Objeto social**: redacción precisa con CNAE asociado.

7. **Domicilio social**. Marcar implicación competencia Registro
   Mercantil.

8. **Pacto de socios opcional**: si los socios quieren regular
   relaciones internas → derivar a `/robin:societario-pacto-socios`.

9. **Trámites postnotariales**:
   - Solicitud de NIF provisional → definitivo.
   - Alta en censo fiscal.
   - Inscripción en Registro Mercantil del domicilio.
   - Alta en Seguridad Social (TGSS) si tendrá empleados.
...
Entregables:
- Borrador de estatutos (texto íntegro)
- Acta fundacional / escritura matriz
- Checklist de trámites post-constitución
- Calendario con fechas críticas

## Avisos típicos

- Si hay socios extranjeros UE: simplificación; si extracomunitarios:
  comprobar autorización inversión exterior si umbral.
- Sociedad profesional (LSP): forma específica + estatutos especiales.
- Régimen económico matrimonial del socio casado en gananciales:
  carácter de las participaciones.
