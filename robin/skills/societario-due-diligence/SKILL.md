---
name: societario-due-diligence
description: >
  Due diligence legal exhaustiva para compraventa de SL/SA. Cubre societario,
  laboral, fiscal, mercantil, IP, datos, contencioso y compliance. Identifica
  red flags y deal breakers.
argument-hint: "[target + perímetro DD + sector + tamaño]"
---

# /robin:societario-due-diligence

Pipeline:

1. **`mcp__robin__preparar_due_diligence`** con los datos del target.
   Devuelve checklist exhaustiva por área.

2. **Áreas a cubrir** (cada una llama tools específicas):

   **Societario** — `mcp__robin__preparar_acuerdo_societario` (gap
   detection en libros de actas, encadenamiento de poderes, capital).

   **Laboral** — `mcp__robin__buscar_convenio_colectivo` para
   convenio aplicable; plantilla, contingencias FOGASA, sanciones
   ITSS, contratos directivos.

   **Fiscal** — `mcp__robin__buscar_teac` por la entidad si tiene
   contencioso fiscal; comprobar BINs, créditos fiscales, actas en
   curso.

   **Mercantil/contractual** — contratos top-5 clientes, top-5
   proveedores, financieros. Cláusulas de cambio de control.

   **IP/IT** — marcas (OEPM/EUIPO), patentes, software, contratos
   cloud, propiedad del código.

   **Protección de datos** — `mcp__robin__buscar_aepd` por la
   entidad para detectar brechas notificadas o sanciones.

   **Contencioso** — pleitos activos, prescripciones, contingencias.

   **Compliance** — `mcp__robin__auditar_compliance_penal_corporativo`,
   `_aml`, `_whistleblowing`, `_rgpd`.

3. **Manifestaciones y garantías (R&W)**: lista de R&W que el
   vendedor debe asumir, indemnidades específicas para los hallazgos.

4. **Deal breakers vs. condicionantes vs. price adjustments**:
   clasificar cada hallazgo.

5. **Verifica todas las citas** que aparezcan en el informe.
...
Resumen ejecutivo
[5-7 líneas: estado general, deal breakers, recomendación closing]

Hallazgos por severidad
🔴 CRÍTICO (deal breaker)
  1. […]

🟠 ALTO (condición precedente al closing)
  1. […]

🟡 MEDIO (price adjustment o R&W reforzada)
  1. […]
...
Manifestaciones y garantías recomendadas
[catálogo de R&W con redacción propuesta]

Indemnidades específicas
[para hallazgos concretos no resolubles antes del closing]
