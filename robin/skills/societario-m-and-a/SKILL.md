---
name: societario-m-and-a
description: >
  Estructuración y documentación de operación M&A: term sheet, SPA / APA,
  manifestaciones y garantías, indemnidades, condiciones precedentes, cierre.
argument-hint: "[tipo de operación + partes + perímetro + estructura preferida]"
---

# /robin:societario-m-and-a

Pipeline:

1. **Estructura de la operación**:
   - Share deal (compraventa de participaciones/acciones) vs. asset
     deal (compraventa de activos).
   - Implicaciones fiscales (`mcp__robin__buscar_dgt` consultas
     vinculantes sobre tipo de operación).
   - Notificación a CNMC si superan los umbrales de concentración
     (`mcp__robin__buscar_cnmc`).

2. **Documentación**:
   - **Term sheet** (preliminar).
   - **SPA (Share Purchase Agreement) / APA (Asset Purchase Agreement)**:
     `mcp__robin__preparar_documento_ma` con la estructura elegida.
   - **Manifestaciones y garantías** del vendedor (financieras,
     laborales, fiscales, regulatorias, contractuales, IP).
   - **Indemnidades específicas** para contingencias identificadas en
     DD.
   - **Condiciones precedentes** al closing.
   - **Carve-outs** si aplican.
   - **Escrow / earn-out** si hay diferimiento de precio.

3. **Cláusulas críticas**:
   - Limitación de responsabilidad (importes, plazos).
   - Tax matters.
   - Non-compete y non-solicit del vendedor.
   - Ley aplicable y jurisdicción / arbitraje.
   - Cambio de control en contratos clave.

4. **Closing checklist**:
   - Documentos a aportar al cierre (poderes, certificaciones,
     consentimientos de terceros).
   - Inscripciones registrales necesarias post-closing.

5. **Verifica citas**.

## Formato

Bloque por documento. Las redacciones complejas (M&G, indemnidades)
se entregan como cláusulas con comentario.

## Avisos típicos

- Si el target opera en sector regulado (financiero, seguros, energía,
  telecos), aprobación del regulador puede ser CP.
- Si hay empleados clave, considerar acuerdos de continuidad y
  retention bonus.
- IVA en asset deals: tratamiento como rama de actividad ex art. 7.1º
  LIVA si concurren requisitos.
