---
name: sucesiones-particion
description: >
  Partición hereditaria judicial o extrajudicial. Cálculo de legítimas según
  derecho aplicable (foral o CC). Liquidación, adjudicación, inscripción.
argument-hint: "[causante + herederos + bienes + foral o común]"
---

# /robin:sucesiones-particion

Pipeline:

1. **Foral-check OBLIGATORIO**. La ley sucesoria es la del estado
   personal del causante (vecindad civil). Cataluña, Galicia, Aragón,
   Navarra, País Vasco, Baleares, Valencia (Ley 6/2024): reglas
   propias muy distintas del CC estatal.

2. **Calcular legítimas**. `mcp__robin__calcular_herencia`:
   - CC estatal: legítima 2/3 (1/3 estricta + 1/3 mejora).
   - Cataluña: 25 % global, no familiar concreta.
   - Galicia: legítima reducida.
   - Aragón: legítima colectiva.
   - Navarra: libertad de testar (legítima formal).
   - País Vasco: legítima 1/3 troncal en Bizkaia/Llodio/Aramaio.
   - Baleares: 1/3 reducida.

3. **`mcp__robin__preparar_escrito_sucesiones`** con tipo=particion.

4. **Modalidades**:
   - Notarial extrajudicial (acuerdo unánime herederos).
   - Judicial por contador-partidor (art. 1057 CC).
   - Judicial contenciosa.

5. **Liquidación impuesto sucesiones**: comunidad autónoma del
   causante (residencia habitual último 5 años).

6. **Doctrina TS Sala 1ª y forales**.

7. **Verifica citas**.

## Avisos típicos

- Si causante extranjero residente España: Reglamento UE 650/2012
  (Sucesorio Europeo).
- Conflicto entre herederos con vecindades distintas: prevalece la
  del causante.
